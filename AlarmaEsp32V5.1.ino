...
void resetWiFiCredentials() {
  ...
  WiFi.disconnect(true, true); // Desconectar y limpiar EEPROM
  
  // Limpiar la lista de redes guardadas
  preferences.begin("wifi_nets", false);
  preferences.clear();
  preferences.end();
  
  delay(1000);
  notify("WiFi reseteado.\nPortal activado.", 3000);
  Serial.println("[WIFI] ✅ Credenciales limpias - Abriendo portal");
  
  // Lanzar portal en tarea
...
void portalTask(void *pvParameters) {
  ...
  Serial.printf("[WIFI] PortalTask: portal finalizado (connected=%d)\n", connected);
  
  // Si se conectó exitosamente, guardar las credenciales
  if (connected && WiFi.status() == WL_CONNECTED) {
    String ssid = WiFi.SSID();
    String password = WiFi.psk();
    Serial.printf("[WIFI] ✅ Conectado a través del portal: %s\n", ssid.c_str());
    if (ssid.length() > 0) {
      saveWiFiNetwork(ssid.c_str(), password.c_str());
      notify("✅ Red guardada:\n" + ssid, 2000);
    }
